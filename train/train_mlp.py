...
Train a fully-connected MLP regressor on J1–J2 energies.

This script ties together:
- QuantumJ1J2Solver (physics)
- Feature extraction (angles or xyz)
- MLP model
...
from models.mlp_regressor import J1J2MLPRegressor
from utils.feature_utils import make_inputs_from_angles
...
def main():
    ...
    # 1. Configuration
    # -----------------------------
    MODEL_FEATURE_MODE = "angles"   # "angles" or "xyz"
    ...
    x_all, input_dim = make_inputs_from_angles(theta, phi, mode=MODEL_FEATURE_MODE)
    y_all = energies

    print(f"Using MLP with feature mode = '{MODEL_FEATURE_MODE}', input_dim = {input_dim}")
    ...
    print("\nFinal MLP test performance (best model):")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("MSE Loss")
    plt.title(f"MLP ({MODEL_FEATURE_MODE}, {OPTIMIZER_NAME}) Training and Validation Loss")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"plots/mlp_{MODEL_FEATURE_MODE}_{OPTIMIZER_NAME}_loss_curve.png")
    plt.close()
    ...
    plt.title(f"MLP ({MODEL_FEATURE_MODE}, {OPTIMIZER_NAME}) Validation MAE")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"plots/mlp_{MODEL_FEATURE_MODE}_{OPTIMIZER_NAME}_val_mae.png")
    plt.close()

    print("MLP training curves saved to 'plots/' directory.")
